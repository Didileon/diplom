...
def user_index(request):
    ...
    return render(request, '/user_index.html', {'cars': cars})
...
def user_index(request):
    cars = models.Car.objects.filter(user=request.user)
    return render(request, 'main/user_index.html', {'cars': cars})
...
class MaintenanceListView(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = super().get_queryset().filter(car__user=car__user)  # фильтрация по текущему пользователю
        self.filterset = CarFilter(self.request.GET, queryset)
        return self.filterset.qs
...
def maintenance_view(request):
    maintenances = models.Maintenance.objects.filter(car__user=request.user)
...
class ComplaintListView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.GET.get('clear'):
            self.request.session.pop('order_by3')
            self.request.session.pop('fn')
            self.request.session.pop('rm')
            self.request.session.pop('sc')
    ...
    def get_queryset(self):
        car__user = self.request.user.id
        queryset = super().get_queryset().filter(car__user=car__user)  # фильтрация по текущему пользователю
        self.filterset = CarFilter(self.request.GET, queryset)
        return self.filterset.qs
...
def complaints_view(request):
    complaints = models.Complaint.objects.filter(car__user=request.user)
